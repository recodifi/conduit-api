...
router.get('/', auth.optional, function (req, res, next) {
  ...
  Promise.all([
    ...
    return Promise.all([
      ...
      let user = results[2];
      return res.json({
        articles: articles.articles.map(function (article) {
...
router.param('comment', function (req, res, next, slug) {
...
router.get('/:article/comments', auth.optional, function (req, res, next) {
  Promise.resolve(req.payload ? User.findById(req.payload.id) : null).then(function (user) {
    return req.article.populate({
      ...
      return res.json({comment: req.article.comments.map(function (comment) {
        return commnet.toJSONFor(user);
      })});
    });
  }).catch(next);
});
